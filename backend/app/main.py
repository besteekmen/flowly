...
Memory = Annotated[Store, Depends(get_store)]
...
async def rate_limit(request: Request, store: Memory):
...
def create_app() -> FastAPI:
    app = FastAPI(title="Flowly v1 API", version="1.0.0")
    ...
    async def sign_up(body: m.SignUp, store: Memory):
    ...
    async def sign_in(body: m.SignIn, store: Memory):
    ...
    async def google(body: m.GoogleSignIn, store: Memory):
    ...
    async def reset_password(body: m.ResetPassword, store: Memory):
    ...
    async def change_password(body: m.ChangePassword, auth: Auth, store: Memory):
        user, token = auth
    ...
    @app.patch("/api/me")
    async def update_profile(body: m.UpdateProfile, auth: Auth):
        user = auth[0]
        user.update(body.model_dump(exclude_unset=True))
        return user

    @app.delete("/api/me", status_code=204)
    async def delete_account(auth: Auth, store: Memory):
        user_id = auth[0]["id"]
        board_ids = {b["id"] for b in store.boards.values() if b["ownerId"] == user_id}
        store.tasks = {k: t for k, t in store.tasks.items() if t["boardId"] not in board_ids}
        store.boards = {k: b for k, b in store.boards.items() if k not in board_ids}
        store.revoke_sessions(user_id)
        store.clear_resets(user_id)
        store.passwords.pop(user_id, None)
        store.users.pop(user_id)
        return Response(status_code=204)

    @app.get("/api/board")
    async def get_board(auth: Auth, store: Memory):
        return store.board_for(auth[0]["id"])

    @app.patch("/api/board")
    async def rename_board(body: m.RenameBoard, auth: Auth, store: Memory):
        board = store.board_for(auth[0]["id"])
        board["name"] = body.name.strip() or "My Board"
        return board

    @app.get("/api/board/tasks")
    async def list_tasks(auth: Auth, store: Memory):
        return store.board_tasks(store.board_for(auth[0]["id"])["id"])

    @app.post("/api/board/tasks", status_code=201)
    async def create_task(body: m.CreateTask, auth: Auth, store: Memory):
        board = store.board_for(auth[0]["id"])
        now = timestamp()
    ...
    async def update_task(taskId: str, body: m.UpdateTask, auth: Auth, store: Memory):
    ...
    async def delete_task(taskId: str, auth: Auth, store: Memory):
        task = owned_task(store, auth[0], taskId)
    ...
    async def move_task(taskId: str, body: m.MoveTask, auth: Auth, store: Memory):
        task = owned_task(store, auth[0], taskId)
